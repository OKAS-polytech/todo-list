...
** *処理フロー*:
. `+DatabaseManager.initializeDatabase()+`を呼び出す。
. Swingのイベントディスパッチスレッドで、Model, View, Controllerをインスタンス化する。
. `+controller.initView()+`を呼び出し、初期データを表示させる。
. Viewのウィンドウを可視化する。

== 4. 処理フロー (メモ保存の例)
. ユーザーが`+JList+`でタスクを選択する。
. `+GuiController+`に登録された`+ListSelectionListener+`がイベントを検知する。
. `+GuiController+`は選択された`+Task+`オブジェクトを取得し、そのメモを`+TodoFrame+`の`+JTextArea+`に表示するよう指示する。
. ユーザーが`+JTextArea+`の内容を編集し、「メモ保存」ボタンをクリックする。
. `+GuiController+`に登録された`+ActionListener+`がイベントを検知する。
. `+GuiController+`は選択中のタスクIDと`+JTextArea+`のテキストを取得し、`+TodoService.updateMemo(id, newMemo)+`を呼び出す。
. `+TodoService+`が`+TaskRepository+`を通じてデータベースの該当レコードを更新する。
. `+GuiController+`が`+refreshTaskList()+`を呼び出し、画面表示を最新の状態に更新する。
