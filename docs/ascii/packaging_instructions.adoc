...
. link:https://repo1.maven.org/maven2/org/xerial/sqlite-jdbc/[Maven Central Repository]にアクセスします。
. 最新バージョンのディレクトリを選択し、`+sqlite-jdbc-X.X.X.X.jar+`ファイルをダウンロードします。
. プロジェクトのルートディレクトリに`+lib+`フォルダを作成し、ダウンロードしたJARファイルを配置します。
+
----
my-todolist-project/
├── lib/
